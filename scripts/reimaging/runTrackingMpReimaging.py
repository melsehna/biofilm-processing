...
indexCsv = '/mnt/data/reimaging/index/reimaging_processed_index.csv'

featVersion = 'trackingVec_v2'
...
def trackColoniesAllFrames(rawStack, maskStack, seedFrame, peakFrame, plateId, wellId):
    ...
    for t in range(seedFrame + 1, nFrames):
        if peakFrame is not None and prevT < peakFrame:
            effRadius = propRadiusPx * (t - prevT)
        else:
            effRadius = propRadiusPx
...
def processOneWell(row):
    ...
    try:
        ...
        maskStack = loadMaskStack(row['maskPath'])

        biomassCsv = row['biomassPath']
        
        nFrames = rawStack.shape[2]
        frames = list(range(nFrames))
        
        if biomassCsv and os.path.exists(biomassCsv):
        ...
        if seedFrame is not None and biomassCsv and os.path.exists(biomassCsv):
            tsDf = pd.read_csv(biomassCsv)

            if 'biomass' in tsDf.columns and len(tsDf):
                peakFrame = int(tsDf['biomass'].idxmax())
        ...
        outdir = os.path.dirname(row['processedPath'])
        ensureDir(outdir)
...
def main():
    ...
    # optional exclusion
    if 'geneName' in indexDf.columns:
        df = indexDf[~indexDf['geneName'].isin(excludeMuts)]
    else:
        df = indexDf.copy()
